    ...
    return (
        <Dialog.Root>
            <Dialog.Trigger className='rounded-md text-left flex flex-col bg-slate-800 p-5 gap-3 overflow-hidden relative outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400'>
                ...
                <span className='text-sm font-medium text-slate-300 text-transform: capitalize'>{formatDistanceToNow(note.date, { locale: ptBR, addSuffix: true })}</span>
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className='overflow-hidden fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-[640px] w-full h-[60vh] bg-slate-700 rounded-md flex flex-col ontline-none'>
                    ...
                    <button type='button' className='w-full bg-slate-800 py-4 tex-center text-sm text-slate-300 outline-none font-medium group'>Deseja <span className='text-red-400 group-hover:underline'>apagar essa nota?</span></button>
                </Dialog.Content>
            </Dialog.Portal>
        </Dialog.Root>
    )
